...
* Variants of onboarding / modes of operation:
    ...
    - **[Variant A]** - generates `"MASTER" MNEMONIC` for a _**user**_ and allows
...
* _**server**_:
    ...
    - receive `G_PUBLIC` and `C_UUID` from _**client**_ and store them
...
* _**server**_:
    ...
    - receive `S_KEY` and `TX_PAYLOAD` from _**client**_
    ...
    - if the extraction has failed it means that received `S_KEY` is invalid,
        so potential data breach occured on _**client**_ side or simply
        _**user**_ made a typo in `KEY`. In such scenario next steps
        are impossible, so the procedure has to be aborted,
        _failure counter_ has to be increased and _**client**_ has
        to be notified. Security precautions can be implemented in this step,
        for example after `3` failed attempts communication with thi
        _**client**_ can be suspended for a longer period of time, etc.
    ...
    - send `C_PASSPHRASE` and `S_SIGNATURE` to _**client**_
